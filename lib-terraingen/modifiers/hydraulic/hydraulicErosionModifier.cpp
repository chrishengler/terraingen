...
namespace tg {
    namespace modify {
        ...
        Vector2<double> HydraulicErosionModifier::calculateGradient(
          ...
          double length =
              std::sqrt(gradient.x * gradient.x + gradient.y * gradient.y);
          if (length > 0) {
            gradient.x /= length;
            gradient.y /= length;
          }
        ...
        bool
        HydraulicErosionModifier::isInBounds(const Vector2<double> &pos) const {
          return pos.x >= 0 && pos.x < heightmap.size() - 1 && pos.y >= 0 &&
                 pos.y < heightmap[0].size() - 1;
        }

        double HydraulicErosionModifier::getInterpolatedHeight(
            const Vector2<double> &pos) const {
        ...
        void HydraulicErosionModifier::deposit(const Vector2<double> &pos,
                                               double amount) {
          int x0 = static_cast<int>(pos.x);
          int y0 = static_cast<int>(pos.y);
          double fx = pos.x - x0;
        ...
        void HydraulicErosionModifier::erode(const Vector2<double> &pos,
                                             double amount) {
        ...
        void HydraulicErosionModifier::simulateParticle(Particle &particle) {
          while (particle.lifetime > 0 && isInBounds(particle.position)) {
            ...
            particle.velocity.x =
                particle.velocity.x * params.inertia +
                gradient.x * params.gravity * (1 - params.inertia);
            particle.velocity.y =
                particle.velocity.y * params.inertia +
                gradient.y * params.gravity * (1 - params.inertia);

            std::uniform_real_distribution<double> dist(-0.1, 0.1);
            particle.velocity.x += dist(rng);
            particle.velocity.y += dist(rng);

            // Moving by velocity leads to weird artifacts from skipping
            // squares, Normalize to ensure no squares skipped
            ...
            if (heightDiff > 0) {
              double deposit_amount = std::min(heightDiff, particle.sediment);
              particle.sediment -= deposit_amount;
              deposit(particle.position, deposit_amount);
            } else {
              double speed = particle.velocity.magnitude();
              double capacity = std::max(speed * params.sedimentCapacity, 0.01);

              double erosion = std::min(capacity - particle.sediment,
                                        -heightDiff * params.erosionRate);
              if (erosion > 0) {
                erode(particle.position, erosion);
