...
namespace tg {
    namespace generate {
        int get_required_grid_size(const Vector2<uint32_t> &dimensions)
        {
        ...
        Heightmap generate_ds(const DiamondSquareParameters &params)
        {
            ...
            for(uint32_t step = grid_size - 1; step > 1; step /= 2) {
                uint32_t half_step = step / 2;

                for(uint32_t x = half_step; x < grid_size - half_step; x += step) {
                    for(uint32_t y = half_step; y < grid_size - half_step; y += step) {
                ...
                for(uint32_t x = 0; x < grid_size; x += half_step) {
                    for(uint32_t y = (x + half_step) % step; y < grid_size; y += step) {
                        double sum = 0.0;
                        int count = 0;
        ...
        std::unique_ptr<Heightmap> generate_ds_unique(const DiamondSquareParameters &params) {
            return std::make_unique<Heightmap>(generate_ds(params));
        }
    }
}